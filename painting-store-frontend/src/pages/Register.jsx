...
export default function Register(){
  ...
  const submit = async (e) => {
    ...
    try{
      const res = await api.register({ name, email, password, adminCode: adminCode || undefined })
      login(res.data)
      navigate('/')
    }catch(err){
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
      ...
      <form onSubmit={submit} className="space-y-3">
        <input value={name} onChange={e=>setName(e.target.value)} required placeholder="Full name" className="w-full px-3 py-2 border rounded" />
        ...
        <button type="submit" disabled={loading} className="w-full px-3 py-2 bg-indigo-600 text-white rounded">{loading?'Creating...':'Create account'}</button>
      </form>
    </div>
  )
}